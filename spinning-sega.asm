...
HV_COUNTER:				EQU		$00C00008						;H/V COUNTER
...
MAIN:					LEA		VDP_CONTROL,A0					;A0 = VDP_CONTROL PORT
						...
						LEA		HV_COUNTER,A2					;A2 = H/V COUNTER
						MOVE.W	#$8134,(A0)						;DISABLE DISPLAY TO TRANSFER GRAPHICS DATA QUICKLY

						MOVEQ.L	#15,D0							;16 WORDS TO WRITE
						LEA		SEGA_PALETTE,A3					;PALETTE DATA AT A3
						MOVE.L	#$C0000000,(A0)					;TELL VDP TO WRITE TO CRAM
WRITE_PALETTE:			MOVE.W	(A3)+,(A1)						;TRANSFER COLOR TO CRAM
						DBRA	D0,WRITE_PALETTE				;LOOP THROUGH COLORS
						
						MOVE.W	#((SEGA_PALETTE-SEGA_LOGO)/2)-1,D0	;NUMBER OF TILES, IN WORDS - 1, TO TRANSFER
						LEA		SEGA_LOGO,A3					;LOAD POINTER TO SEGA LOGO TILES
						MOVE.L	#$40000000,(A0)					;TELL VDP TO WRITE TO VRAM
WRITE_LOGO:				MOVE.W	(A3)+,(A1)						;WRITE TILE DATA TO VRAM
						DBRA	D0,WRITE_LOGO
						
						MOVE.L	#($2000/2)-1,D0					;D0 = NUMBER OF WORDS TO WRITE TO PLANE A MAP
						MOVE.L	#$60000002,(A0)					;TELL VDP TO WRITE TO VRAM PLANE A
						LEA		SEGA_TILEMAP,A3					;A3 = ADDRESS OF SEGA TILEMAP
WRITE_PLANEA:			MOVE.W	(A3)+,(A1)						;MOVE TILEMAP INFO TO PLANE A
						DBRA	D0,WRITE_PLANEA					;KEEP WRITING TO PLANE A
						
						MOVE.W	#$8174,(A0)						;ENABLE THE DISPLAY
						
						MOVE.W	#$8F00,(A0)						;0 AUTO INCREMENT
						MOVE.L	#$40000010,(A0)					;SET VDP TO WRITE TO VSRAM 0x0000
						MOVEQ.L	#82,D0							;D0.L = SCANLINE 82 WHICH IS WHERE SEGA LOGO STARTS
						MOVE.L	#140,D1							;D1.L = SCANLINE 140 WHICH IS WHERE SEGA LOGO ENDS
RESET_V_POSITION:		MOVEQ.L	#140-83,D2						;D2.L = NUMBER OF SCANLINES IN THE SEGA LOGO
						MOVE.W	#0,(A1)							;START PLANE A AT POSITION Y = 0
WAIT_FOR_TOP:			MOVE.B	(A2),D3							;D3.B = POSITION OF V COUNTER
						CMP.B	D3,D0							;CHECK VERTICAL POSITION AGAINST TOP OF SEGA LOGO
						BNE		WAIT_FOR_TOP					;CHECK IF V COUNTER IS NOT AT TOP OF SEGA LOGO
DRAW_LINE:				MOVE.W	D2,(A1)							;DRAW THE PROPER LINE OF THE SEGA LOGO
						SUBQ.L	#2,D2							;MOVE LOGO DOWN AND DRAW NEXT LINE
						CMP.B	D3,D1							;CHECK IF LAST LINE
						BEQ		RESET_V_POSITION				;IF LAST LINE THEN RESTART!
						ADDQ.L	#1,D3							;NEXT LINE
WAIT_NEXT_LINE:			CMP.B	(A2),D3							;CHECK FOR NEXT LINE
						BNE		WAIT_NEXT_LINE					;IF NOT NEXT LINE THEN WAIT
						BRA		DRAW_LINE						;IF NEXT LINE THEN DRAW
